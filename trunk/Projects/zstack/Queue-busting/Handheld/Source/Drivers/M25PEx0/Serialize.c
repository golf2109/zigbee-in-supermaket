...
#define SPI_SETUP(spi, baudRate, options)           \
   do {                                             \
      ...
      if(options & SPI_SLAVE){                      \
         ...
         U##spi##GCR = 15;/*BAUD_E(baudRate, CLKSPD);*/    \
         U##spi##BAUD = 0;/*BAUD_M(baudRate);*/           \
      }                                             \
...
#else
  #define SLAVE_CHIP_SELECT()     (P1 &= 0xF7)
  #define SLAVE_CHIP_DESELECT()   (P1 |= 0x08)
...
void DF_spiInit( int8 (**readWrite)(uint8, uint32, uint8 *, uint16))
{
  ...
  if (!s_xmemIsInit)  {
    ...
    // deselect the part.
    P1SEL &= ~BV(3);    // set P1.4 as GPIO pin
    P1DIR |=  BV(3);  // set P1.4 as output
    P1    |=  BV(3);  // set output high
  }
...
/*
    if(enumNull == opt)
      return;
*/
    if((unsigned char)opt & (unsigned char)MaskBit_SelectSlave_Relevant)  
      ((unsigned char)opt & (unsigned char)MaskBit_SlaveSelect) ? SLAVE_CHIP_SELECT() : SLAVE_CHIP_DESELECT();
/**
